...
AI topic detection uses the official `@openrouter/sdk` package from the backend. It sends only compact metadata for newly inserted articles: title and short description, plus the internal article id needed to map results back. Full article bodies, provider RSS categories, and source names are not sent to the model. Local fallback topics are produced by weighted phrase scoring with positive/negative evidence instead of broad substring matching, with regression fixtures for difficult Italian headlines. Classifier requests disable model reasoning and request JSON-object responses with topic confidence, while optional evidence can still be stored when available. AI topics are accepted only when the topic is canonical and confidence is high enough. The existing local/RSS-derived taxonomy is used as an immediate fallback and news ingestion returns without waiting for the AI request; when AI returns at least one valid canonical topic, the backend replaces the fallback topics in the background. Each stored topic includes source, confidence, evidence, and reason-code metadata, and each article stores AI-processing metadata so attempted articles, including no-topic results, are not reprocessed on later refreshes or service restarts. Admins can inspect classification details with `GET /internal-api/admin/articles/:articleId/topics/debug`.
