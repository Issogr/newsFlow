...
AI topic detection uses the official `@openrouter/sdk` package from the backend. It sends only compact metadata for newly inserted articles: source, title, and short description. Full article bodies and provider RSS categories are not sent to the model. Classifier requests disable model reasoning and request JSON-object responses so the output budget is reserved for the final taxonomy JSON. The prompt includes short taxonomy guidance so Cronaca means incidents, crime, public order, courts, and injuries rather than only local news. The existing local/RSS-derived taxonomy is used as an immediate fallback and news ingestion returns without waiting for the AI request; when AI returns at least one valid canonical topic, the backend replaces the fallback topics in the background. Each article stores AI-processing metadata so attempted articles, including no-topic results, are not reprocessed on later refreshes or service restarts.
