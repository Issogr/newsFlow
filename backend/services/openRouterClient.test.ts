...
describe('openRouterClient', () => {
  ...
  test('does not let an older in-flight success clear a newer backoff', async () => {
    let resolveOlderRequest!: (value: { choices: Array<{ finishReason: string; message: { content: string } }> }) => void;
    const olderResponse = new Promise<{ choices: Array<{ finishReason: string; message: { content: string } }> }>((resolve) => {
