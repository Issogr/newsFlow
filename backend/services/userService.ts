...
let pendingAuthenticatedPublicApiRequests = new Map<string, { count: number; usedAt: string }>();
...
function isUniqueConstraintError(error: unknown) {
  const databaseError = error as AppError;
  return String(databaseError?.code || '').startsWith('SQLITE_CONSTRAINT')
    || String(databaseError?.message || '').toLowerCase().includes('unique constraint failed');
}
...
function normalizeEnumValue(value: unknown, allowedValues: Set<string>, fallback: string) {
...
function buildSetupLink(pathName: string, token: string) {
...
function getPasswordSetupExpiryDate(ttlMinutes: number) {
...
function isUserOnline(lastActivityAt: string | null | undefined) {
  if (!lastActivityAt) {
    return false;
  }
...
function buildAdminUserSummary(user: UserRecord) {
  return {
    id: user.id,
...
function getPasswordSetupTokenRecord(rawToken: unknown): PasswordSetupTokenRecord {
...
function buildCurrentUserResponse(user: UserRecord) {
...
function buildAuthResponse(user: UserRecord, sessionToken: string) {
...
function mapApiTokenForUser(tokenRecord: DynamicRecord | null | undefined) {
  if (!tokenRecord) {
    return null;
  }
...
function getUserApiToken(userId: string) {
  return mapApiTokenForUser(database.getLatestActiveApiTokenForUser(userId));
}

function normalizeUserSettingsPayload(
  payload: DynamicRecord = {},
  currentSettings: ResolvedUserSettings = getDefaultSettings(),
  ...
  } as ResolvedUserSettings;
}

async function registerUser(payload: DynamicRecord = {}) {
  ...
  try {
    ...
    if ((error as AppError).code === 'USER_ALREADY_EXISTS' || isUniqueConstraintError(error)) {
      throw createError(409, 'Username already exists', 'USER_ALREADY_EXISTS');
    }
    throw error;
  }
...
async function loginUser(payload: DynamicRecord = {}) {
  const username = sanitizeUsername(payload.username);
  const password = String(payload.password || '');
...
function logoutUser(sessionToken: string) {
...
function createApiTokenLabel(label: unknown) {
  return String(label || '').trim().slice(0, 80);
}

function validateInteractiveFeedUrl(url: string, options: AbortOptions = {}) {
...
function createUserApiToken(userId: string, options: { label?: unknown } = {}) {
...
function revokeUserApiToken(userId: string) {
  return database.revokeApiTokensByUserId(userId, new Date().toISOString()) > 0;
}

function updateUserSettings(userId: string, payload: DynamicRecord = {}) {
...
async function addUserSource(userId: string, payload: DynamicRecord = {}, options: AbortOptions = {}) {
  ...
  try {
    ...
    if (isUniqueConstraintError(error)) {
      throw createError(409, 'This RSS source already exists for the user', 'SOURCE_ALREADY_EXISTS', error instanceof Error ? error : new Error(String(error)));
    }
    throw error;
  }
...
async function updateUserSource(userId: string, sourceId: string, payload: DynamicRecord = {}, options: AbortOptions = {}) {
...
async function previewUserSource(payload: DynamicRecord = {}, options: AbortOptions = {}) {
  const url = String(payload.url || '').trim();
  ...
  try {
    ...
    options.signal?.throwIfAborted();
    throw createError(400, 'RSS URL is not valid or cannot be parsed', 'INVALID_RSS_URL', error instanceof Error ? error : new Error(String(error)));
  }
}

function removeUserSource(userId: string, sourceId: string) {
  database.getDb().transaction(() => {
...
function exportUserSettings(userId: string) {
  const settings = getUserSettings(userId);
  const customSources: SourceDefinition[] = database.listUserSources(userId);
...
function getPasswordSetupTokenDetails(rawToken: unknown) {
...
async function completePasswordSetup(payload: DynamicRecord = {}) {
...
function listUsersForAdmin() {
  ...
  const users = database.listUsers()
    ...
    .sort((left: ReturnType<typeof buildAdminUserSummary>, right: ReturnType<typeof buildAdminUserSummary>) => {
  ...
  return {
    ...
    summary: {
      ...
      onlineUsers: users.filter((user: DynamicRecord) => user.isOnline).length,
      activeUsers: users.filter((user: DynamicRecord) => user.lastActivityAt).length,
...
function recordPublicApiRequestUsage({
  authenticated = false,
  userId = null,
  usedAt = new Date().toISOString()
}: { authenticated?: boolean; userId?: string | null; usedAt?: string } = {}) {
...
function createUserPasswordSetupLink(adminUserId: string, targetUserId: string) {
...
async function importUserSettings(userId: string, payload: DynamicRecord = {}, options: AbortOptions = {}) {
  const importedSettings = payload.settings && typeof payload.settings === 'object'
    ? payload.settings as DynamicRecord
    : {};
  ...
  await mapWithConcurrency(importedCustomSources, 4, async (source: DynamicRecord) => {
    ...
    try {
      ...
      options.signal?.throwIfAborted();
      throw createError(400, `Imported RSS URL is not valid: ${url}`, 'INVALID_RSS_URL', error instanceof Error ? error : new Error(String(error)));
    }
  });

  const now = new Date().toISOString();
  const recreatedSources = importedCustomSources.map((source: DynamicRecord) => {
    return {
      id: createId(),
      userId,
  ...
  const excludedGlobalSourceIds = Array.isArray(importedSettings.excludedSourceIds)
    ? importedSettings.excludedSourceIds.filter((sourceId: string) => globalSourceIds.has(sourceId))
    : [];
  const excludedSubSourceIds = Array.isArray(importedSettings.excludedSubSourceIds)
    ? importedSettings.excludedSubSourceIds.filter((sourceId: string) => groupedSubSourceIds.has(sourceId))
    : [];
