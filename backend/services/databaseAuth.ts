...
function createAuthRepository({ getDb }: { getDb: () => SqliteDatabase }) {
  function normalizeTouchIntervalSeconds(minIntervalSeconds: unknown) {
  ...
  function touchUserActivity(userId: string, activityAt: string, minIntervalSeconds = 60) {
    if (!userId) {
      return 0;
    }
  ...
  function updateUserPassword(userId: string, passwordHash: string | null, updatedAt: string) {
    if (!userId) {
      return 0;
    }
  ...
  function createUserSession(session: DynamicRecord = {}) {
    getDb().prepare(`
  ...
  function createApiToken(token: DynamicRecord = {}) {
    getDb().prepare(`
  ...
  function mapApiTokenRow(row: DynamicRecord | undefined | null) {
    if (!row) {
      return null;
    }
  ...
  function getLatestActiveApiTokenForUser(userId: string) {
    if (!userId) {
      return null;
    }
  ...
  function findActiveApiTokenByHash(tokenHash: string) {
    if (!tokenHash) {
      return null;
    }
  ...
  function revokeApiTokensByUserId(userId: string, revokedAt?: string) {
    if (!userId) {
      return 0;
    }
  ...
  function touchApiTokenUsage(tokenId: string, usedAt: string) {
    if (!tokenId) {
      return 0;
    }
  ...
  function incrementUserPublicApiUsage(userId: string, usedAt: string, amount = 1) {
    if (!userId || !usedAt) {
      return 0;
    }
  ...
  function findSessionByTokenHash(tokenHash: string) {
    if (!tokenHash) {
      return null;
    }
  ...
  function refreshSessionExpiry(tokenHash: string, expiresAt: string) {
  ...
  function deleteSessionByTokenHash(tokenHash: string) {
    if (!tokenHash) {
      return 0;
    }
  ...
  function deleteSessionsByUserId(userId: string) {
    if (!userId) {
      return 0;
    }
  ...
  function createPasswordSetupToken(token: DynamicRecord = {}) {
    getDb().prepare(`
  ...
  function findPasswordSetupTokenByHash(tokenHash: string) {
    if (!tokenHash) {
      return null;
    }
  ...
  function markPasswordSetupTokenUsed(tokenHash: string, usedAt: string) {
    if (!tokenHash) {
      return 0;
    }
  ...
  function deleteUnusedPasswordSetupTokens({
    userId = null,
    purpose = null,
