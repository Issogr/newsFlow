...
function getTokenOverlapScore(left: Partial<NewsArticle> = {}, right: Partial<NewsArticle> = {}) {
...
function hasTopicOverlap(left: Partial<NewsArticle> = {}, right: Partial<NewsArticle> = {}) {
...
function getAiCandidateLimit(limit: unknown) {
...
function getCandidateScore(overlapScore: number, topicOverlap: boolean) {
  return overlapScore + (topicOverlap ? 0.1 : 0);
}

function filterCandidateArticles(target: NewsArticle, candidates: NewsArticle[] = [], options: { limit?: number } = {}) {
...
function buildArticlePayload(article: Partial<NewsArticle> = {}) {
  return {
...
function buildPrompt(target: NewsArticle, candidates: NewsArticle[] = []) {
  return [
...
function normalizeMatches(payload: unknown, candidateIds = new Set<string>()) {
  const response = payload && typeof payload === 'object' ? payload as DynamicRecord : {};
  const matches = Array.isArray(response.matches) ? response.matches as DynamicRecord[] : [];
  return matches
    .map((match) => ({
...
function buildStoryGroupId(articleIds: string[] = []) {
...
async function findSimilarStoriesForArticle(target: NewsArticle, candidates: NewsArticle[] = [], options: { limit?: number } = {}) {
...
function getCandidateSignature(target: NewsArticle, candidates: NewsArticle[] = [], options: { limit?: number } = {}) {
