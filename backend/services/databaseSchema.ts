...
function createDatabaseSchema({ logger }: { logger: typeof appLogger }) {
  ...
  function getConfiguredSourceUrlKeys() {
    return new Set(configuredSources.map((source: SourceDefinition) => normalizeArticleUrl(source.url || '') || String(source.url || '').trim()).filter(Boolean));
  }

  function ensureCaseInsensitiveUsernameUniqueness(database: SqliteDatabase) {
  ...
  function initializeLegacySchemaV15(database: SqliteDatabase) {
    database.exec(`
  ...
  function triggerExists(database: SqliteDatabase, triggerName: string) {
  ...
  function getColumnNames(database: SqliteDatabase, tableName: string): Set<string> {
    if (!tableExists(database, tableName)) {
      return new Set();
    }

    return new Set(database.prepare<{ name: string }>(`PRAGMA table_info(${tableName})`).all().map((column) => column.name));
  }

  function hasExistingApplicationSchema(database: SqliteDatabase) {
    return ['articles', 'users', 'user_settings', 'user_sources'].some((tableName) => tableExists(database, tableName));
  }

  function inferLegacySchemaVersion(database: SqliteDatabase) {
  ...
  function setCurrentSchemaVersion(database: SqliteDatabase, version = CURRENT_SCHEMA_VERSION) {
    database.prepare(`
  ...
  function migrateSchemaStep(database: SqliteDatabase, currentVersion: number) {
    ...
    if (currentVersion === 39) {
      const settingsColumns = getColumnNames(database, 'user_settings');
      if (settingsColumns.has('article_retention_hours')) {
  ...
  function assertSupportedSchemaVersion(currentVersion: number) {
    if (
  ...
  function migrateSchema(database: SqliteDatabase, currentVersion: number) {
  ...
  function ensureSupportedSchema(database: SqliteDatabase) {
    ...
    if (legacySchemaVersion === null) {
      database.transaction(() => {
        ...
        setCurrentSchemaVersion(database, MIN_SUPPORTED_SCHEMA_VERSION);
        migrateSchema(database, MIN_SUPPORTED_SCHEMA_VERSION);
        // The legacy baseline retained this unused column; fresh installs never exposed it.
        database.exec('ALTER TABLE user_settings DROP COLUMN auto_refresh_enabled');
      })();
