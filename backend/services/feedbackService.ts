...
interface TelegramResponse extends DynamicRecord {
  description?: string;
  ok?: boolean;
...
function formatAttachmentLabel(attachmentType: AttachmentType) {
...
async function readTelegramResult(response: Response) {
  ...
  const payload = rawPayload && typeof rawPayload === 'object' ? rawPayload as TelegramResponse : null;
...
function buildAttachmentCaption(user: AuthUser, attachmentType: AttachmentType) {
...
async function sendTextMessage(config: TelegramConfig, message: string, signal: AbortSignal) {
...
async function sendAttachment(config: TelegramConfig, user: AuthUser, attachment: Express.Multer.File, signal: AbortSignal) {
  ...
  formData.append(
    ...
    new globalThis.Blob([new Uint8Array(attachment.buffer)], { type: attachmentMimeType }),
    attachmentName
  );
...
async function sendFeedback({
  ...
  try {
    ...
    throw createError(502, 'Unable to deliver feedback right now. Please try again later.', 'FEEDBACK_DELIVERY_FAILED', error instanceof Error ? error : new Error(String(error)));
  }
}
