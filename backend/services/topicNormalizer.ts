...
type WeightedPhrase = [string, number];
type TopicRule = { positive: WeightedPhrase[]; negative: WeightedPhrase[] };
...
function getArticleText(article: Partial<NewsArticle> = {}) {
...
function scoreRules(text: string, rules: WeightedPhrase[] = []) {
...
function classifyTopicsFromText(article: Partial<NewsArticle> = {}, options: { threshold?: number } = {}) {
