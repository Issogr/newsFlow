...
function mapUserSourceRow(row: DynamicRecord): SourceDefinition {
  return {
    ...row,
...
function normalizeReadThematicSummaryIds(summaryIds: unknown[] = []) {
...
function deleteOwnedArticles(database: SqliteDatabase, ownerId: string, sourceId = '') {
...
function getUserSettingsValues(userId: string, settings: UserSettings = {}, updatedAt = new Date().toISOString()) {
  return [
    userId,
...
function getUserSourceValues(source: SourceDefinition, userId = source.userId) {
  return [
    source.id,
    userId,
    source.name,
...
function createUserStateRepository({ getDb }: { getDb: () => SqliteDatabase }) {
  function getUserSettings(userId: string): UserSettings | null {
    ...
    if (!row) {
      ...
      return {
        ...
        compactNewsCardsMode: String(row.compactNewsCardsMode || (row.compactNewsCards ? 'everywhere' : 'off')),
  ...
  function upsertUserSettings(userId: string, settings: UserSettings = {}) {
  ...
  function listUserSources(userId: string) {
    if (!userId) {
      return [];
    }
  ...
  function createUserSource(source: SourceDefinition) {
    getDb().prepare(USER_SOURCE_INSERT_SQL).run(...getUserSourceValues(source));
  }

  function findUserSourceById(userId: string, sourceId: string) {
    if (!userId || !sourceId) {
      return null;
    }
  ...
  function updateUserSource(userId: string, sourceId: string, updates: Partial<SourceDefinition>) {
    if (!userId || !sourceId) {
      return 0;
    }
  ...
  function deleteArticlesForUserSource(userId: string, sourceId: string) {
    if (!userId || !sourceId) {
      return 0;
    }

    const database = getDb();
    const transaction = database.transaction((ownerId: string, customSourceId: string) => {
  ...
  function deleteUserSource(userId: string, sourceId: string) {
    if (!userId || !sourceId) {
      return 0;
    }
  ...
  function deleteAllUserSources(userId: string) {
    if (!userId) {
      return 0;
    }

    const database = getDb();
    const transaction = database.transaction((ownerId: string) => {
  ...
  function listReadThematicSummaryIds(userId: string) {
    if (!userId) {
      return [];
    }
  ...
  function markThematicSummariesRead(userId: string, summaryIds: unknown[] = []) {
    ...
    `);
    const transaction = database.transaction((ownerId: string, ids: string[]) => {
  ...
  function importUserState(userId: string, sources: SourceDefinition[] = [], settings: UserSettings = {}) {
    if (!userId) {
      return {
        settings: null,
        customSources: []
    ...
    const transaction = database.transaction((ownerId: string, importedSources: SourceDefinition[], nextSettings: UserSettings) => {
      deleteOwnedArticles(database, ownerId);
