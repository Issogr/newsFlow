...
interface FeedGroup extends DynamicRecord {
  cursorId?: string;
  items: FeedArticle[];
  pubDate: string;
}
...
interface FeedFilters extends DynamicRecord {
  beforeId?: string;
...
interface AvailableSource extends DynamicRecord {
  ...
  name: string;
  subSources: SourceGroup['subSources'];
}

interface FilterStatsArgs {
...
interface FeedRuntime extends DynamicRecord {
  getLastRefreshAt?: () => string | null;
  getManualRefreshMeta?: () => DynamicRecord;
...
const filterStatsCache = new Map<string, { cachedAt: number; value: DynamicRecord }>();

function cloneJsonSafe<T>(value: T): T {
  return JSON.parse(JSON.stringify(value));
}

function normalizeSignatureList(values: unknown = []) {
...
function buildAvailableSourcesSignature(availableSources: AvailableSource[] = []) {
  return availableSources.map((source) => ({
    id: source.id,
    name: source.name,
    language: source.language || null,
    iconUrl: source.iconUrl || '',
    subSources: (Array.isArray(source.subSources) ? source.subSources : []).map((subSource: DynamicRecord) => [subSource.id, subSource.name, subSource.language || null])
  }));
}
...
function getPagination(filters: FeedFilters = {}) {
  return {
...
function expandUserSources(userSources: SourceDefinition[] = []) {
  return userSources
...
function getQueryOptions(userContext: UserContext = {}): QueryOptions {
  return {
...
function collectGroupArticleIds(groups: FeedGroup[] = []) {
  return groups.flatMap((group) => (group.items || []).map((item: FeedArticle) => item.id).filter(Boolean));
}

function buildNextCursor(groups: FeedGroup[] = [], existingExcludeArticleIds: string[] = []) {
...
function compareFeedPosition(left: DynamicRecord = {}, right: DynamicRecord = {}) {
...
function annotateReadLaterGroups(groups: FeedGroup[] = [], userId: string | null = null) {
  ...
  const articleIds = groups.flatMap((group) => (group.items || []).map((item: FeedArticle) => item.id).filter(Boolean));
  const readLaterArticleIds = database.getReadLaterArticleIdSet(userId, articleIds);

  return groups.map((group) => {
    const groupReadLaterArticleIds = (group.items || [])
      .map((item: FeedArticle) => item.id)
      .filter((articleId: string) => readLaterArticleIds.has(articleId));
...
function compareReadLaterPosition(left: DynamicRecord = {}, right: DynamicRecord = {}) {
...
function buildReadLaterGroup(group: FeedGroup): FeedGroup {
  const savedAt = (group.items || []).reduce((latest: string, item: FeedArticle) => {
  ...
  return {
    ...group,
    readLater: true,
    readLaterArticleIds: (group.items || []).map((item: FeedArticle) => item.id).filter(Boolean),
    readLaterSavedAt: savedAt
  };
}

function fetchGroupedReadLaterPage(filters: FeedFilters = {}, queryOptions: QueryOptions = {}, page = 1, pageSize = 12) {
...
function getArticleCursor(articles: FeedArticle[] = []) {
...
function getOldestReturnedGroupTimestamp(groups: FeedGroup[] = [], pageStart = 0, pageSize = 12) {
  const returnedGroups = groups.slice(pageStart, pageStart + pageSize);
  if (returnedGroups.length === 0) {
    return null;
  }

  return returnedGroups.reduce<number | null>((oldestTimestamp, group) => {
...
function hasScannedGroupingWindow(articles: FeedArticle[] = [], groups: FeedGroup[] = [], pageStart = 0, pageSize = 12) {
  ...
  if (oldestReturnedTimestamp === null || !Number.isFinite(lastFetchedTimestamp)) {
    return false;
  }
...
function fetchGroupedNewsPage(filters: FeedFilters = {}, queryOptions: QueryOptions = {}, page = 1, pageSize = 12) {
  const articles: FeedArticle[] = [];
  let cursor = {
...
function buildSourceCatalogResponse(availableSources: AvailableSource[] = []) {
  return availableSources.map((source) => ({
    id: source.id,
    name: source.name,
    language: source.language || null,
...
function buildFilterStatsCacheKey({ filters = {}, queryOptions = {}, availableSources = [], readLater = false, refreshVersion = null }: FilterStatsArgs) {
...
function buildFilterStatsValue({ filters = {}, queryOptions = {}, availableSources = [], readLater = false }: FilterStatsArgs): DynamicRecord {
  return {
...
function getFilterStats({ filters = {}, queryOptions = {}, availableSources = [], readLater = false, refreshVersion = null }: FilterStatsArgs) {
...
async function getNewsFeed(filters: FeedFilters = {}, userContext: UserContext = {}, runtime: FeedRuntime = {}) {
  const {
...
async function getReadLaterFeed(filters: FeedFilters = {}, userContext: UserContext = {}) {
