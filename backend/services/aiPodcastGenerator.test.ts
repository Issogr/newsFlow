...
function createTestWavBuffer(byteLength = 2048): Buffer {
...
function createTestPcmBuffer(byteLength = 2400, sampleValue = 1000): Buffer {
...
interface AudioHttpResponse {
  status: number;
  headers: { 'content-type': string };
  data: Buffer;
}

function createAudioHttpResponse(
  data: Buffer,
  { contentType = 'audio/pcm', status = 200 }: { contentType?: string; status?: number } = {}
): AudioHttpResponse {
  return {
    status,
...
describe('aiPodcastGenerator', () => {
  ...
  function setPodcastTestEnv(overrides: NodeJS.ProcessEnv = {}): void {
