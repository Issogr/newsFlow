...
function parseLimitedCsvParam(value: unknown, limit: number) {
  return parseCsvParam(value).slice(0, limit);
}

function parseBoundedCsvParam(value: unknown, { limit, name }: { limit: number; name: string }) {
...
function parseBoundedPositiveInteger(value: unknown, fallback: number, max: number) {
...
function parseOptionalBoundedPositiveInteger(value: unknown, max: number) {
...
function parseNewsQuery(query: Record<string, unknown> = {}): NewsQuery {
  return {
    ...
    beforePubDate: typeof query.beforePubDate === 'string' ? query.beforePubDate : '',
    beforeId: typeof query.beforeId === 'string' ? query.beforeId : '',
