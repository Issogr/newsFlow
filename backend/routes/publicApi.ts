...
function getBearerTokenCandidate(req: Request) {
...
function rememberInvalidToken(tokenHash: string, now = Date.now()) {
  invalidTokenCache.set(tokenHash, now + (15 * 60 * 1000));
  while (invalidTokenCache.size > INVALID_TOKEN_CACHE_MAX_ENTRIES) {
    const oldestTokenHash = invalidTokenCache.keys().next().value;
    if (oldestTokenHash === undefined) {
      break;
    }
    invalidTokenCache.delete(oldestTokenHash);
  }
}

function resolveExternalApiPrincipal(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    ...
    if (tokenHash && authError.status === 401) {
      rememberInvalidToken(tokenHash);
      req.externalApiAuthError = authError;
      next();
      return;
    }
    next(error);
...
function rejectInvalidExternalApiToken(req: Request, res: Response, next: NextFunction) {
  next(req.externalApiAuthError);
}

function requirePublicApiModeEnabled(req: Request, res: Response, next: NextFunction) {
  const bearerToken = getBearerTokenCandidate(req);
...
function getExternalUserContext(req: Request) {
