...
- removed podcast briefings, script/audio generation, playback, and configuration; SQLite schema 46 cleans up stored podcast data
...
- moved AI summary and podcast briefing windows to `08:00` and `20:00` local time
...
- prevented podcast scripts from repeating the same news when one article appears across multiple topics or duplicate article identities
- improved podcast script and narration prompts so briefings use stronger editorial selection, smoother transitions, and a more natural single-narrator delivery
...
- kept new-article notices visible during background topic/story updates and blocked those updates during first-run source setup
