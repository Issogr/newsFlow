...
export function getGroupMergeKeys(group = {}) {
  const keys = new Set();
...
export function groupSharesAnyKey(group, keySet) {
...
export const mergeGroups = (primaryGroups, secondaryGroups) => {
  const mergedGroups = [];
...
export function getLoadedNewsGroups(currentGroups, {
  append,
  isReadLaterView,
  maxRetainedGroups,
  mergedItems,
...
export const getSourceReloadSignature = (excludedSourceIds, excludedSubSourceIds, customSources) => JSON.stringify({
  excludedSourceIds,
