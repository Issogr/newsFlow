...
const en: TranslationDictionary = {
  ...
  findRssFeeds: 'Find RSS feeds',
  findingRssFeeds: 'Finding RSS feeds...',
  rssDiscoveryHelp: 'Enter a website address and News Flow will look for its published RSS or Atom feeds.',
  rssFeedsFound: ({ count }) => `${count} RSS feed${Number(count) === 1 ? '' : 's'} found`,
  noRssFeedsFound: 'No RSS or Atom feeds were found on this website.',
  rssFeedResult: 'RSS feed',
  selectRssFeed: ({ title }) => `Select ${title}`,
  deselectRssFeed: ({ title }) => `Deselect ${title}`,
  rssFeedAlreadyAdded: 'Already added',
  addSelectedFeeds: ({ count }) => `Add selected (${count})`,
  addingSelectedFeeds: 'Adding selected feeds...',
  rssUrl: 'Feed URL',
  ...
  remove: 'Remove',
  userMenu: 'User',
  sourcesShown: 'Manage News Sources',
  ...
  adminOverview: 'Account overview',
  adminOnlineHelp: ({ minutes }) => `Online means active in the last ${minutes} minutes.`,
  adminAutoRefresh: ({ seconds }) => `Updates every ${seconds} seconds.`,
  adminUsersTitle: 'Users',
  adminAccount: 'Admin',
  adminAccountProtected: 'Cannot be deleted',
  adminApiActivity: 'API activity',
  adminActions: 'Actions',
  adminUserActions: ({ username }) => `Actions for ${username}`,
  adminSetupLinkReadyFor: ({ username }) => `Password reset link for ${username}`,
  adminResetLinkHelp: 'Share this link with the user so they can choose a new password.',
  adminCopyLinkError: 'Could not copy the link. Select it and copy it manually.',
  ...
  adminUsersUnavailable: 'Could not load users. Try refreshing.',
  ...
  summaryOutdated: 'Outdated briefing',
