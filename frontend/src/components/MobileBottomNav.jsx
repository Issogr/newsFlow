...
function FilterBubble({ children, onClose, title, open }) {
  return (
    <div
    ...
    >
      {title && (
        <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
          <span className="text-sm font-semibold uppercase tracking-[0.16em] text-slate-500">{title}</span>
          <button
            type="button"
            onClick={onClose}
      ...
      <div ref={navRef} className="relative mx-auto w-[calc(100%-1.25rem)] max-w-md pb-2.5">
        ...
        <div className="relative h-[3.95rem] overflow-hidden">
          ...
          >
            <button
              type="button"
              onMouseDown={(event) => handleBubbleButtonPress(event, 'sources')}
              onTouchStart={(event) => handleBubbleButtonPress(event, 'sources')}
            ...
            <button
              type="button"
              onMouseDown={(event) => handleBubbleButtonPress(event, 'topics')}
              onTouchStart={(event) => handleBubbleButtonPress(event, 'topics')}
          ...
          <div
            className={`absolute inset-0 flex items-center gap-2 px-2.5 py-2 transition-all duration-300 ease-out ${
              searchMode
                ? 'translate-x-0 scale-100 opacity-100 blur-0'
                : 'pointer-events-none translate-x-8 scale-95 opacity-0 blur-sm'
            }`}
          >
            <div className="flex flex-1 items-center gap-2 rounded-xl border border-slate-200/80 bg-slate-50 px-3 py-2 shadow-inner">
              ...
              {search && (
                <button
                  ...
                  className="inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-slate-400 transition-colors hover:bg-slate-200 hover:text-slate-600"
            ...
            <button
              ...
              className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-800"
              aria-label={t('cancel')}
            >
              <X className="h-5 w-5" />
            </button>
