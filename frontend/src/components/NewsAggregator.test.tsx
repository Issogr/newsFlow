...
import { fetchNews as fetchNewsImplementation, fetchReadLaterNews as fetchReadLaterNewsImplementation, fetchThematicSummaries as fetchThematicSummariesImplementation, isRequestCanceled as isRequestCanceledImplementation, markThematicSummariesRead as markThematicSummariesReadImplementation, updateUserSettings as updateUserSettingsImplementation } from '../services/api';
...
vi.mock('../services/api', () => ({
  ...
  isRequestCanceled: vi.fn((error: unknown) => Boolean(error && typeof error === 'object' && 'code' in error && error.code === 'ERR_CANCELED'))
}));

const fetchNews = vi.mocked(fetchNewsImplementation);
const fetchReadLaterNews = vi.mocked(fetchReadLaterNewsImplementation);
const fetchThematicSummaries = vi.mocked(fetchThematicSummariesImplementation);
const isRequestCanceled = vi.mocked(isRequestCanceledImplementation);
const markThematicSummariesRead = vi.mocked(markThematicSummariesReadImplementation);
const updateUserSettings = vi.mocked(updateUserSettingsImplementation);
const useTopicRefreshSocket = vi.mocked(useTopicRefreshSocketImplementation);
...
vi.mock('./NewsCard', () => {
  const getRenderedTopicSummary = (group: Partial<NewsGroup> = {}) => {
    const topicEntries = [
  ...
  return {
    default: ({ group, onOpenReader }: { group: NewsGroup; onOpenReader: (group: NewsGroup, articleId?: string) => void }) => (
      <div>
        <div>{group.title}</div>
...
vi.mock('./ReaderPanel', () => ({
  default: ({ group }: { group: NewsGroup }) => <div data-testid="reader-item-count">{group.items?.length || 0}</div>
}));
vi.mock('./BrandMark', () => ({
  default: () => <div />
}));
...
type NewsAggregatorProps = ComponentProps<typeof NewsAggregator>;

async function renderNewsAggregator(overrides: Partial<NewsAggregatorProps> = {}) {
  let view: RenderResult | undefined;
  const renderedUser = overrides.currentUser || currentUser;
  const locale: Locale = overrides.locale || renderedUser.settings.defaultLanguage || 'en';
...
function createGroups(prefix: string, start: number, count: number): NewsGroup[] {
...
function createRetitledGroups(idPrefix: string, titlePrefix: string, start: number, count: number): NewsGroup[] {
...
function createGroup(id: string, title: string, pubDate = '2026-03-14T10:00:00.000Z'): NewsGroup {
  return {
    id,
    title,
...
function createFeedResponse(items: NewsGroup[] = [], overrides: Partial<FeedResponse> = {}): FeedResponse {
  return {
    items,
...
function createSingleGroupFeedResponse(id: string, title: string, options: Partial<FeedResponse> & { pubDate?: string } = {}) {
  const { pubDate, ...overrides } = options;
  return createFeedResponse([createGroup(id, title, pubDate)], overrides);
}

function createThematicSummary(overrides: Partial<ThematicSummary> = {}): ThematicSummary {
...
function captureSocketHandlers() {
  const socketHandlers: Partial<TopicRefreshSocketOptions> = {};
...
async function dispatchSocketHandler(handler: ((payload: NewsUpdatePayload) => unknown) | undefined, payload: NewsUpdatePayload) {
  if (!handler) throw new Error('Socket handler not captured');
  await act(async () => {
...
function setLoadMoreIntersection(isIntersecting: boolean) {
  const observer = intersectionObservers.at(-1);
  expect(observer).toBeDefined();
  act(() => observer!.callback([{ isIntersecting, target: observer!.target } as IntersectionObserverEntry], observer as unknown as IntersectionObserver));
}
...
describe('NewsAggregator', () => {
  let desktopMediaQuery: Pick<MediaQueryList, 'matches' | 'addEventListener' | 'removeEventListener'>;
  ...
  test('shows card-shaped placeholders while the initial feed loads', async () => {
    const request = createDeferred<FeedResponse>();
  ...
  test('keeps the latest news response when an older request resolves later', async () => {
    const firstRequest = createDeferred<FeedResponse>();
    const secondRequest = createDeferred<FeedResponse>();
    let callCount = 0;
  ...
  test('renders thematic summary stories and opens the summary panel', async () => {
    ...
    expect(screen.queryByRole('heading', { name: 'Storie per topic' })).not.toBeInTheDocument();
  ...
  test('ignores stale thematic summary responses after a newer refresh', async () => {
    const socketHandlers = captureSocketHandlers();
    const firstSummariesRequest = createDeferred<Awaited<ReturnType<typeof fetchThematicSummariesImplementation>>>();
    const secondSummariesRequest = createDeferred<Awaited<ReturnType<typeof fetchThematicSummariesImplementation>>>();
    ...
    let refreshPromise: void | Promise<void>;
    await act(async () => {
  ...
  test('shows one-time source setup and excludes unselected sources and sub-feeds', async () => {
    ...
    let socketOptions!: TopicRefreshSocketOptions;
    const setupUser = {
      ...currentUser,
      settings: {
        ...currentUser.settings,
  ...
  test('replaces stale fallback topics after a silent AI topic reload', async () => {
    ...
    fetchNews
      ...
      .mockResolvedValueOnce(createFeedResponse([{
        ...
        }], { filters: { topics: [{ topic: 'Tecnologia' }] } }));
  ...
  test('keeps pending new article notice after silent topic reloads', async () => {
    ...
    fetchNews
      ...
      .mockResolvedValueOnce(createFeedResponse([
        ...
        ], { meta: { hasMore: true, totalGroups: 2 }, filters: { topics: [{ topic: 'Technology' }] } }));
  ...
  test('does not cancel manual refresh loading when a silent topic reload arrives', async () => {
    const socketHandlers = captureSocketHandlers();
    const manualRefreshRequest = createDeferred<FeedResponse>();
    ...
    fetchNews.mockImplementation(({ beforeId, pageSize }) => {
      ...
      if (pageSize === 13) {
        return Promise.resolve(createFeedResponse([
          ...
          ], { meta: { pageSize: 13, nextCursor: null }, filters: { topics: [{ topic: 'Technology' }] } }));
      }
  ...
  test('preserves the loaded article count when a topic refresh lands right after load more resolves', async () => {
    const socketHandlers = captureSocketHandlers();
    const appendRequest = createDeferred<FeedResponse>();
  ...
  test('restores automatic pagination when a list reload cancels an append', async () => {
    const appendRequest = createDeferred<FeedResponse>();
    const reloadRequest = createDeferred<FeedResponse>();
    let callCount = 0;
