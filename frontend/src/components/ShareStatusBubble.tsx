import { AlertCircle, Check } from 'lucide-react';
import type { Translator } from '../types';

type ShareState = 'idle' | 'copied' | 'failed';

function getShareStatusPresentation(shareState: ShareState, t: Translator) {
  if (shareState === 'copied') {
    return {
      Icon: Check,
      text: t('shareCopiedMessage'),
      className: 'border-emerald-200 bg-emerald-50 text-emerald-800',
      iconClassName: 'bg-white text-emerald-700',
    };
  }

  if (shareState === 'failed') {
    return {
      Icon: AlertCircle,
      text: t('shareFailedMessage'),
      className: 'border-amber-200 bg-amber-50 text-amber-800',
      iconClassName: 'bg-white text-amber-700',
    };
  }

  return null;
}

const ShareStatusBubble = ({ shareState, t, className = '' }: { shareState: ShareState; t: Translator; className?: string }) => {
  const presentation = getShareStatusPresentation(shareState, t);

  if (!presentation) {
    return null;
  }

  const { Icon, text, className: bubbleClassName, iconClassName } = presentation;

  return (
    <div
      className={`pointer-events-none inline-flex whitespace-nowrap items-center gap-2 rounded-full border px-3 py-2 text-xs font-medium shadow-lg ${bubbleClassName} ${className}`.trim()}
      role="status"
      aria-live="polite"
    >
      <span className={`inline-flex h-5 w-5 items-center justify-center rounded-full ${iconClassName}`}>
        <Icon className="h-3.5 w-3.5" />
      </span>
      <span>{text}</span>
    </div>
  );
};

export default ShareStatusBubble;
