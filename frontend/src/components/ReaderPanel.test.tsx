...
import { fetchReaderArticle as fetchReaderArticleImplementation, updateUserSettings as updateUserSettingsImplementation } from '../services/api';
import { createDeferred, resolveDeferred } from '../test-utils/deferred';
import type { ComponentProps } from 'react';
import type { CurrentUser, NewsGroup, ReaderResponse, Translator } from '../types';
...
const fetchReaderArticle = vi.mocked(fetchReaderArticleImplementation);
...
function buildReaderPanel(props: Partial<ComponentProps<typeof ReaderPanel>> = {}) {
  return (
    <ReaderPanel
      group={group}
...
function renderReaderPanel(props: Partial<ComponentProps<typeof ReaderPanel>> = {}) {
  return render(buildReaderPanel(props));
}

function selectSourceVersion(articleId: string) {
  fireEvent.change(screen.getByRole('combobox', { name: 'sourceVersions' }), {
    target: { value: articleId }
  });
}

function createReaderPayload(text = 'Body', overrides: Partial<ReaderResponse> = {}): ReaderResponse {
  return {
    title: 'Reader title',
    language: 'en',
    excerpt: 'Excerpt',
...
describe('ReaderPanel', () => {
  ...
  test('keeps the latest article payload when an older reader request resolves later', async () => {
    const firstRequest = createDeferred<ReaderResponse>();
    const secondRequest = createDeferred<ReaderResponse>();
  ...
  test('shows text-shaped loading feedback while reader content loads', async () => {
    const request = createDeferred<ReaderResponse>();
